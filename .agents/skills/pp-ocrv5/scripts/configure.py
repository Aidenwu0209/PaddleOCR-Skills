...
def save_config(api_url: str, token: str, project_root: Path, quiet: bool = False) -> bool:
    ...
    if env_file.exists():
        ...
        # Preserve other configuration items (excluding old and new PP-OCRv5 keys)
            ...
            for line in f:
                ...
                if line and not line.startswith('#'):
                    if '=' in line:
                        ...
                        key = key.strip()
                        # Skip old and new PP-OCRv5 keys (will be overwritten)
                        if key not in ['API_URL', 'PADDLE_OCR_TOKEN', 'AISTUDIO_HOST',
