...
1. **ONLY use PP-OCRv5 API** - Execute the script `python scripts/pp-ocrv5/ocr_caller.py`
...
2. **Execute OCR**:
   ```bash
   python scripts/pp-ocrv5/ocr_caller.py --file-url "URL provided by user" --pretty
   ```
   Or for local files:
   ```bash
   python scripts/pp-ocrv5/ocr_caller.py --file-path "file path" --pretty
   ```

   **Save result to file** (recommended):
   ```bash
   python scripts/pp-ocrv5/ocr_caller.py --file-url "URL" --output result.json --pretty
   ```
...
**URL OCR**:
```bash
python scripts/pp-ocrv5/ocr_caller.py --file-url "https://example.com/invoice.jpg" --pretty
```
...
5. **Configure automatically**:
   ```bash
   python scripts/pp-ocrv5/configure.py --api-url "PARSED_URL" --token "PARSED_TOKEN"
   ```
...
- `references/pp-ocrv5/output_schema.md` - Output format specification
- `references/pp-ocrv5/provider_api.md` - Provider API contract
...
```bash
python scripts/pp-ocrv5/smoke_test.py
```
