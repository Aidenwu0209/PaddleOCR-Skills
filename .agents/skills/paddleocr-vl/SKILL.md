...
{
  "ok": true,
  "text": "Complete text with all content including headers, footers, etc.",
  "result": {
    "layout": {
      "regions": [
        {
...
| "Complete document" | Everything | Use all regions or text field |
...
2. **Tell user to provide credentials**:
   ```
   Please visit the URL above to get your PADDLEOCR_VL_API_URL and PADDLEOCR_VL_TOKEN.
   Once you have them, send them to me and I'll configure it automatically.
   ```

3. **When user provides credentials** (accept any format):
   - `PADDLEOCR_VL_API_URL=https://xxx.com/v1, PADDLEOCR_VL_TOKEN=abc123...`
   - `Here's my API: https://xxx and token: abc123`
   - Copy-pasted code format
   - Any other reasonable format

4. **Parse credentials from user's message**:
   - Extract PADDLEOCR_VL_API_URL value (look for URLs)
   - Extract PADDLEOCR_VL_TOKEN value (long alphanumeric string, usually 40+ chars)
...
```bash
# .env
PADDLEOCR_VL_MAX_FILE_SIZE_MB=50  # Increase from 20MB to 50MB
```
...
def extract_complete(json_response):
    # Simply use the text field which includes everything
    print(json_response['text'])

    # Or present all regions in order from result
    regions = json_response['result']['layout']['regions']
    reading_order = json_response['result']['layout']['reading_order']
...
- `references/paddleocr-vl/output_schema.md` - Output format specification
- `references/paddleocr-vl/provider_api.md` - Provider API contract

> **Note**: Model version and capabilities are determined by your API endpoint (PADDLEOCR_VL_API_URL).
